...
export default function ShowDetail() {
  ...
  const getStatusDot = (status, color) => {
    ...
    switch (status) {
      ...
      case "ยกเลิกการเคลมสินค้า":
        return (
          <TbBasketCancel twoToneColor="#FF0000" style={{ fontSize: 22 }} />
        );
      default:
        return null;
    }
  ...
  const timelineItems = useMemo(() => {
    // Check if "ยกเลิกการเคลมสินค้า" exists in the data.
    const hasCancelledStatus = data.some(
      (d) => d.jobStatus === "ยกเลิกการเคลมสินค้า"
    );

    // Create a new statusOrder array based on the check.
    const currentStatusOrder = hasCancelledStatus
      ? statusOrder
      : statusOrder.filter((status) => status !== "ยกเลิกการเคลมสินค้า");

    const total = currentStatusOrder.length;

    const items = currentStatusOrder.map((status, idx) => {
      const record = data.find((d) => d.jobStatus === status);
      const levelColor = getLevelColor(idx, total);
      const soft = getSoftColors(idx, total);

      if (record) {
        const hasDetails = record.remark || (record.images && record.images.length > 0);
            ...
            {record.jobStatus !== "เริ่มงาน" &&
              ...
              record.images.length > 0 && (
                <div>
                  ...
                  >
                    {record.images
                      .filter((img) => img.status === record.jobStatus)
                      .map((img, i) => (
                        <img
                          key={i}
                          src={img.imageUrl}
